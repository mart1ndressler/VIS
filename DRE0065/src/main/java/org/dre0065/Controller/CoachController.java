...
public class CoachController {
    ...
    public ResponseEntity<String> deleteCoach(@PathVariable int id, @RequestParam(defaultValue = "false") boolean withDependencies) {
        ...
        } else if (coachService.hasDependencies(id)) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body("Coach has dependencies in other tables and cannot be deleted.");
        }
    ...
    public String updateCoach(@PathVariable int id, @Valid @RequestBody Coach updatedCoach) {
        return coachService.updateCoachById(id, updatedCoach);
    }
}
