...
public class PreparationService {
            ...
            for (Preparation preparationFromJson : preparationsFromJson) {
                ...
                if (fighter != null && coach != null) {
                    ...
                    if (existingPreparation != null) {
                        ...
                        // Použití Factory Patternu při vytváření instance Preparation
                        Preparation preparationToSave = Preparation.createPreparation(
                                preparationFromJson.getStartOfPreparation(),
                                preparationFromJson.getEndOfPreparation(),
                                preparationFromJson.getMmaClub(),
                                preparationFromJson.getClubRegion(),
                                fighter,
                                coach
                        );
                    ...
                    System.out.println("Fighter or Coach not found for preparation starting on: " + preparationFromJson.getStartOfPreparation());
                }
            }
            System.out.println("Preparations successfully processed from JSON!");
        } catch (IOException e) {
            System.err.println("Error loading preparations from JSON: " + e.getMessage());
        }
    }
    ...
    public void saveAllPreparations(List<Preparation> preparations) {
        // Použití Factory Patternu pro každou preparation před uložením
        ...
        for (Preparation preparation : preparations) {
            ...
            Preparation preparationBuilt = Preparation.createPreparation(
                    preparation.getStartOfPreparation(),
                    preparation.getEndOfPreparation(),
                    preparation.getMmaClub(),
                    preparation.getClubRegion(),
                    fighter,
                    coach
            );
